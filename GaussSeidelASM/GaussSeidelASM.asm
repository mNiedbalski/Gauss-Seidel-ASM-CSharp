...
MyProc1 proc        
    ...
    afterKLoop:         ;first x[j] = sum / equations[j][j] then if (Math.Abs(x[j] - temp) > tolerance) done = false;
            ...
            comiss  xmm0, xmm2                 ;remember that value of x[j] is overwritten by True or False
            jl      Negative      ;jl          ;jump if x[j]-temp<0
    AbsDone:
            shufps  xmm1, xmm1, 39h            ;XMM1 = [temp][x[k]][][tolerance]
            comiss  xmm0, xmm1                 ;remember that value of x[j] is overwritten by True or False
            jg      doneFalse  ;jg             ;if x[j]-temp > tolerance
    incrJ: 
            inc     r15                        ;Incrementing j counter
            cmp     r15, rcx                   ;Checking if loop has finished (loops until j==n)
